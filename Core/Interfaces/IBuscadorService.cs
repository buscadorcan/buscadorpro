...
namespace Core.Interfaces
{
    public interface IBuscadorService
    {
        ...
        int AddEventTracking(EventTrackingDto eventTracking, string ipAddress);

        Task<string> GetCoordinates(string address);

        Task<string> ExportExcel(string paramJSON);

        Task<string> ExportPdf(string paramJSON);
        Task<byte[]> DescargarPDF(string url);
    }
}
